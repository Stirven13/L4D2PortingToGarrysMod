...
11. Нажмите `Ctrl+A` и нажмите на `Apply Pose as Rest Pose`.
12. Нажмите `F3` и в появившемся окне вбейте в поиск `Clear Pose Constraints`, либо можно просто нажать `Ctrl + Alt + C`:  
![Blender F3 Window ClearPoseConstraints](images/blender_f3_window_clear_pose_constraints.png)
1.  Теперь необходимо запустить второй скрипт `F Proportion Script 2`:  
...
1.  Если после запуска у вас появились новую объекты, в которые есть 001, то их следует удалить:
...
1.  После этого вы можете экспортировать файл, при помощи пункта [Как экспортировать Source Engine файлы в Blender](#как-экспортировать-source-engine-файлы-из-blender).
2.  Теперь у вас есть декомпилированная версия модельки для Garry's Mod, но нужно ещё вставить [`default_physics.smd`](#5-default_physicssmd) в папку, куда вы экспортировали файлы из Blender.
3.  Вставьте `*.qc` файл, из папки куда вы декомпилировали модель.
4.  Самым последним этапом нормализации это [Изменение *.QC файла](#изменение-qc-файла), вам необходимо его изменить, чтобы у модельки были корректные анимации.
...
# Изменение *.QC файла
`*.qc` файл можно открыть при помощи блокнота.
## $attachment
...
Всё удаляем.
## $includemodel
Полностью всё удаляем и потом заменяем в зависимости от ситуации. Ниже приведён список на что нужно будет заменять.
### PlayerModel (не NPC)
